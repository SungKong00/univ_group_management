...
### 5. Speckit 임시 문서 제외
- **`specs/` 디렉토리의 모든 파일은 Speckit 작업용 임시 문서입니다.**
- **이 문서들은 유지보수 대상에서 제외**되며, 다음 규칙을 적용하지 않습니다:
  - 100줄 원칙 (예외)
  - 코드 참조 정책 (예외)
  - 컨텍스트 추적 문서 업데이트 (제외)
  - sync-status.md 추적 (제외)
- **용도**: 특정 기능 개발 시 Speckit 워크플로우에서 사용하는 spec.md, plan.md, tasks.md
- **생명주기**: 해당 기능 완료 및 PR 머지 후 삭제 가능
- **주의**: 영구 참조용 문서는 반드시 `docs/` 디렉토리에 작성
