...
대학에서는 하나의 학과 안에서도 공지는 카카오톡, 모집은 구글 폼, 일정은 엑셀, 회의록은 노션으로 흩어져 있다. 동아리나 학회를 운영하는 입장에서는 "우리 그룹만의 공간"이 없어서, 매번 새 단톡방을 만들거나 플랫폼을 옮겨야 한다.

이 프로젝트는 **대학의 조직 구조(대학 → 단과대 → 학과 → 동아리/학회)를 그대로 반영한 통합 플랫폼**이다. 그룹마다 워크스페이스가 자동 생성되고, 채널별로 권한을 세분화할 수 있으며, 모집·일정관리·장소예약까지 하나의 플랫폼에서 처리한다.

백엔드(Spring)만 공부하다가 프론트엔드를 처음 해본 첫 풀스택 프로젝트다.
...
그룹 유형은 6가지(대학, 단과대, 학과, 연구실, 공식, 자율)이며, **각 그룹은 독립적으로 운영**된다. 상위 그룹이 하위 그룹의 내부 활동(채널, 게시글, 멤버)을 감시하거나 통제할 수 없고, 유일하게 "하위 그룹 생성 승인/거절"만 가능하다.
...
그룹이 승인되면 **워크스페이스가 자동 생성**되고, 기본 2개 채널(공지, 자유게시판)이 템플릿으로 만들어진다. 그룹장은 필요에 따라 채널을 추가할 수 있다:
...
새 채널은 **Secure by Default** 원칙에 따라 권한 0으로 시작한다. 그룹장이 명시적으로 "이 역할은 이 채널에서 뭘 할 수 있다"를 설정해야 접근 가능하다.
...
"같은 멤버인데 공지 채널에서는 읽기만, 자유게시판에서는 글쓰기 가능" — 이 요구사항을 해결하기 위해 2계층 권한 모델을 설계했다.

**1계층 — 시스템 역할 (그룹 단위)**: 그룹장(모든 권한), 교수(그룹장 동급), 멤버(기본 참여)

**2계층 — 채널 역할 바인딩 (채널 단위)**: 각 역할이 각 채널에서 어떤 권한(조회/읽기/쓰기/댓글/파일)을 갖는지 개별 설정. 커스텀 역할(운영진, 모집담당 등)도 생성 가능하다.

이 구조에서 가장 고민했던 것은 **권한 체크 성능**이었다. 멤버 수백명이 채널 여러개에 접근할 때마다 DB를 조회하면 N+1 문제가 발생하므로, Caffeine 캐시를 도입해서 최대 2번의 DB 조회로 모든 권한 판단을 완료하도록 설계했다.
...
채널 내에서 게시글을 작성하고, 2단계 댓글(게시글 댓글 → 대댓글)로 토론한다.

- **게시글 고정(Pin)** — 중요 공지를 상단에 고정
- **읽은 위치 기억** — 다시 들어오면 마지막으로 읽은 게시글 위치에서 시작
- **Optimistic UI** — 글 작성 시 서버 응답 전에 즉시 화면에 반영, 실패하면 롤백

이 기능을 Clean Architecture(Domain → Data → Presentation)로 전면 재구성하면서, 기존 코드 대비 관심사 분리와 테스트 커버리지를 크게 개선했다.
...
일정 관리를 3개 레이어로 분리해서, 개인 스케줄과 그룹 활동이 충돌 없이 공존하도록 설계했다.
...
특히 **장소 예약 시스템**은 요일별 운영시간, 공휴일 차단, 관리자 이전 등 실제 대학 환경에서 필요한 세부 기능까지 설계했다.
...
동아리/학회가 새 멤버를 모집할 때의 전체 워크플로우를 시스템화했다.
...
그룹당 활성 모집은 1개만 가능하고, 학생은 하나의 모집에 1회만 지원할 수 있다(철회 후 재지원 가능). 마감 시 자동 종료되며 아카이브로 이동한다.
...
**Clean Architecture 재설계** — 프로젝트가 커지면서 기존 모놀리식 백엔드(`backend/`)의 한계를 느끼고, `backend_new/`에서 6개 도메인으로 Bounded Context를 나눈 Clean Architecture를 설계했다. 29개 Entity, 52개 API, 155개 테스트를 작성하고 7단계 마이그레이션 전략까지 수립했다. 기존 백엔드에 완전히 통합하지는 못했지만, "모놀리식 → 도메인 분리"를 직접 경험하면서 아키텍처 설계 역량이 크게 성장한 과정이었다.

**Navigator 2.0 마이그레이션** — Flutter의 명령형 네비게이션(Navigator 1.0)에서 선언형(Navigator 2.0)으로 전환하면서, 그룹 간 전환 시 발생하는 상태 꼬임 문제를 해결했다. Spring에서는 `@GetMapping`이면 끝나는 라우팅이 프론트엔드에서는 이렇게 복잡한 줄 몰랐다. 98개 테스트를 작성하며 권한 기반 폴백, 딥링크, 히스토리 보존 등의 엣지 케이스를 검증했다.

**프론트엔드 디자인 시스템 실험** — UI 일관성 문제를 해결하기 위해 `frontend_new/`에서 디자인 토큰 + 39개 재사용 컴포넌트를 만드는 실험을 했다. 비즈니스 로직이 있는 앱은 아니고 컴포넌트 라이브러리 쇼케이스다.
