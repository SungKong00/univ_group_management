...
#### 웹 (Desktop/Tablet)
-   워크스페이스 첫 진입 시, **그룹 홈**이 기본으로 표시됩니다.
-   이전에 특정 채널을 보고 있었다면, 새로고침 시 해당 채널로 복원됩니다.

#### 모바일
-   워크스페이스 첫 진입 시, **채널 목록** (채널 네비게이션 뷰)이 기본으로 표시됩니다.
-   이전에 특정 채널을 보고 있었다면, 앱 재시작 시 해당 채널로 복원됩니다.
...
### 6.1. 뷰 타입 유지 전략
그룹 전환 시 사용자가 보던 뷰 타입(groupHome, calendar, channel)을 유지하여 일관된 경험 제공:

- **그룹홈 → 그룹홈**: 다른 그룹의 그룹홈으로 전환
- **캘린더 → 캘린더**: 다른 그룹의 캘린더로 전환
- **채널 → 첫 번째 채널**: 다른 그룹의 첫 번째 채널로 전환 (채널 ID는 유지 불가)

### 6.2. 통합 네비게이션 히스토리
모든 네비게이션 이동(채널, 뷰, 그룹 전환)을 하나의 스택으로 관리:

- **히스토리 추가**: 채널 이동, 뷰 전환, 그룹 변경 시 자동 기록
- **뒤로가기**: 스택에서 이전 상태를 꺼내 완전 복원 (그룹 ID, 뷰, 채널, 게시글, 댓글 표시 여부)
- **중복 방지**: 마지막 히스토리와 동일한 상태는 추가하지 않음
- **그룹 간 이동**: 뒤로가기로 다른 그룹으로 돌아갈 수 있음

### 6.3. 최초 접속 시 동작

#### 웹
- LocalStorage에 저장된 마지막 뷰가 없으면 `WorkspaceView.groupHome`으로 진입
- 권한 검증 후 관리자 페이지 표시 또는 그룹홈으로 폴백
