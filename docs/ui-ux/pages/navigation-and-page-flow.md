...
-   **페이지 정보**:
    -   **위치**: 뒤로가기 버튼 오른쪽.
    -   **내용**: 현재 페이지의 이름(예: "홈", "워크스페이스")을 표시합니다.
...
-   **관리자 (하단)**
    -   현재 사용자가 관리자 권한을 가진 경우에만 **관리자 페이지** 버튼이 표시됩니다.
...
-   상단 바 자체는 새로고침되지 않으며, 페이지 정보만 현재 컨텍스트를 명확히 나타내도록 업데이트됩니다. (예: "워크스페이스 - OOO 그룹")
...
1.  **채널 네비게이션 뷰**: 워크스페이스에 처음 진입하면, 화면 전체가 **채널 목록**으로 표시됩니다. (웹처럼 네비게이션 바와 콘텐츠가 분리되지 않음)
2.  **게시글 뷰**: 채널 목록에서 특정 채널을 선택하면, 화면 전체가 해당 채널의 **게시글 목록**으로 전환됩니다.
