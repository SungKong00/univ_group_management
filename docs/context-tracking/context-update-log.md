...
**추가된 문서**:
- `docs/testing/test-data-reference.md`: TestDataRunner가 생성하는 테스트 데이터 구조 및 사용자/그룹 정보 관리

**배경**:
- TestDataRunner의 코드가 복잡해지면서 어떤 유저가 어느 그룹에 소속되어 있고 어떤 권한을 가지는지 파악이 어려웠음
- 테스트 작성 시 참조할 수 있는 중앙화된 문서 필요

**문서 내용**:
- 테스트 사용자 3명 정보 (이메일, 학번, 소속, 역할)
- 테스트 그룹 정보 (기본 그룹 4개, 커스텀 그룹 2개)
- 사용자별 멤버십 및 권한 매핑
- 테스트 모집 공고 및 지원서 정보
- 테스트 장소 정보 및 사용 승인 현황
- 테스트 시나리오별 활용 예시 (권한/모집/장소 관리)
- 테스트 데이터 확장 가이드

**연관 문서 업데이트**:
- `CLAUDE.md`: 개발 워크플로우 섹션에 테스트 데이터 링크 추가

**관련 파일**:
- `backend/src/main/kotlin/org/castlekong/backend/runner/TestDataRunner.kt` (참조)
