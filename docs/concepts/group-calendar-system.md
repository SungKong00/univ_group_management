...
그룹 캘린더는 **공식 일정(공지형)**과 **비공식 일정**으로 구성됩니다(Phase 1 MVP).
향후 대상 지정형(TARGETED), 참여 신청형(RSVP) 등이 확장될 예정입니다.
멤버들의 참여 상태를 추적하고 시간표 기반 예약 가능 시간을 표시합니다.
...
### 예약 가능 시간 표시 (Phase 1 MVP)
일정 생성 시 **시간표 기반 예약 가능 시간**을 달력에 표시합니다.
충돌 시간은 회색으로 표시되어 일정 조율 시 충돌을 방지합니다.

### 최적 시간 추천 (Phase 2 이후)
대상자들의 **시간표 + 기존 참여 일정**을 분석하여 충돌이 가장 적은 시간대를 슬롯별로 추천합니다 (구현 예정).
사용자는 분석 대상을 토글하여 추천 정확도 조절 가능.

### 채널 연동 (Phase 2 이후)
일정 생성 시 특정 채널을 선택하면 해당 채널에 일정을 설명하는 게시글 자동 생성 (구현 예정):
1. 게시글에 일정 상세정보(시간, 장소) 포함
2. 액션 버튼([참여 신청], [불참 의사]) 포함

## 실제 사용 흐름 (Phase 1 MVP)
