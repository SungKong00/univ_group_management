...
-   **사용 그룹 승인**: 다른 그룹의 '장소 사용 그룹' 신청 승인/거부
-   **예약 관리**: 모든 예약 현황 조회 및 강제 취소
-   **시간 설정**: 예약 가능/불가능 시간 설정 (주 단위 반복 또는 특정 날짜)

### 2.2. 권한 통합 방안

> **설계 의논 필요**: 장소 관리 권한을 기존 RBAC 시스템에 어떻게 통합할지 결정 필요
> - Option A: 새로운 권한 타입 추가 (`PLACE_MANAGE`, `PLACE_RESERVE`)
> - Option B: 그룹 권한과 독립적인 별도 권한 시스템
