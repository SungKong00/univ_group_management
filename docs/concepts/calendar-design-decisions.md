...
**결정**: 캘린더 권한을 기존 RBAC 시스템에 통합하되, CALENDAR_MANAGE 1개만 추가 (Option A 단순화)

**배경**: 캘린더 시스템에 필요한 권한을 기존 그룹 권한 체계에 통합할지, 독립적인 권한 시스템으로 운영할지 결정 필요. 추가로 권한 개수 최소화를 통한 사용성 개선 검토

**고려한 대안**:
- Option A-1: GroupRole에 CALENDAR_MANAGE 1개만 추가 (채택)
- Option A-2: GroupRole에 3개 권한 추가 (CALENDAR_VIEW, CALENDAR_MANAGE, PLACE_RESERVE)
...
- **권한 체계 간소화**: 조회/예약은 멤버십 기반으로 처리하여 별도 권한 불필요

**권한 체크 로직**:
- **캘린더 조회**: 그룹 멤버면 누구나 가능 (`isMember()` 체크, 별도 권한 불필요)
- **장소 예약**: 해당 그룹이 PlaceUsageGroup에 APPROVED 상태면 누구나 가능 (별도 권한 불필요)
- **캘린더 관리**: CALENDAR_MANAGE 권한 필요
  - 공식 일정 생성/수정/삭제
  - 비공식 일정 수정/삭제 (작성자 아니어도 가능)
- **장소 관리**: CALENDAR_MANAGE 권한 필요
  - 장소 등록/수정/삭제
  - 사용 그룹 승인/거절
  - 운영 규칙 설정

**영향**:
- GroupRole에 CALENDAR_MANAGE 1개 권한만 추가
- PermissionService 확장 필요
- 권한 매트릭스 UI에 캘린더 탭 추가
- 조회/예약 권한은 멤버십 및 승인 상태로 자동 판별

**기각된 대안의 문제점**:
- Option A-2: 권한 개수 증가로 사용자 혼란, 실제로는 멤버십만으로 충분한 기능에 불필요한 권한 추가
- Option B: 권한 확인 로직 이원화로 복잡도 증가, 캐싱 전략 분리 필요

---

## DD-CAL-002: 반복 일정 저장 방식 및 UI 설계

**결정**: 명시적 인스턴스 저장 방식 (Option B) + 종료일 기반 범위 지정
...
**반복 범위 지정 방식**: 시작일 + 종료일 기반
- UI에서 "시작일"과 "종료일"을 선택하도록 유도
- 선택한 기간 내에서 반복 패턴에 따라 명시적 인스턴스 생성
...
3. **반복 패턴 선택**:
   - **Option A: "매일" (DAILY)**
     - 시작일부터 종료일까지 매일 인스턴스 생성
   - **Option B: "요일 선택" (WEEKLY)**
     - 체크박스: [월] [화] [수] [목] [금] [토] [일]
     - 선택한 요일에만 인스턴스 생성

**백엔드 저장 방식**:
- 선택한 기간(`start_date` ~ `end_date`) + 반복 패턴에 따라 **명시적 인스턴스 생성**
- 예시:
  - 2025-11-01 ~ 2025-11-30, 매주 월/수/금 선택 → 13개 GroupEvent 레코드 생성
  - 모든 레코드는 동일한 `series_id`를 공유하여 그룹화
- `recurrence_rule` 필드에 JSON 형식으로 저장:
  ```json
  {
    "type": "WEEKLY",
...
**영향**:
- 일정 생성 UI에서 시작일/종료일 DatePicker 2개 필수
- 반복 패턴 선택 UI (매일 라디오 버튼 / 요일 체크박스)
