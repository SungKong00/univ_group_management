...
-   **예약 가능 시간 판별 로직 (상세)**:
    ...
    2.  **2단계: 예약 현황 확인 (Blacklist)**
        -   운영 규칙이 있다면(09:00-18:00), 시스템은 다음으로 `PlaceReservation` 테이블에서 11월 25일에 이미 잡힌 모든 예약을 조회합니다. (예: 10:00-11:00, 15:00-17:00)
...
권한 변경에 따라 각 API의 접근 제어(권한) 요구사항이 업데이트됩니다.
...
-   **그룹 캘린더 API (`/api/groups/{groupId}/events`)**:
    -   `GET /`: **그룹 멤버**
    -   `POST /`: (공식) **`CALENDAR_MANAGE`** / (비공식) **그룹 멤버**
...
-   **장소 API (`/api/places`)**:
    ...
    -   (기타 장소 관리 API는 `PLACE_MANAGE` 권한 필요)