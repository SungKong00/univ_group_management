...
-   **예약 가능 시간 판별 로직 (상세)**:
    ...
    2.  **2단계: 차단 시간 확인**
        -   `PlaceBlockedTime` 테이블에서 11월 25일에 해당하는 차단 시간이 있는지 확인합니다.
        -   차단된 시간대는 운영 시간 내라도 '예약 불가'로 처리됩니다.

    3.  **3단계: 예약 현황 확인 (Blacklist)**
        -   운영 규칙이 있고 차단된 시간이 아니라면, 시스템은 `PlaceReservation` 테이블에서 11월 25일에 이미 잡힌 모든 예약을 조회합니다. (예: 10:00-11:00, 15:00-17:00)
