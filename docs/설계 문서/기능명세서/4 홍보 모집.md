...
### 7. API 명세 (UI/UX 명세서 v1.0 기준 통합)

| 기능 | HTTP Method | URI | Request Body | Response Body | 설명 |
| --- | --- | --- | --- | --- | --- |
| 모집 공고 목록 조회 | GET | /recruitments?query={query}&tags={tags} | | `List<RecruitmentDto>` | 검색/태그 필터링을 포함하여 활성화된 공고 목록을 조회합니다. |
| 모집 공고 상세 조회 | GET | /recruitments/{recruitmentId} | | `RecruitmentDetailDto` | 특정 모집 공고의 상세 정보를 조회합니다. |
| **모집 공고 작성** | **POST** | **/groups/{groupId}/recruitments** | `CreateRecruitmentRequest` | `RecruitmentDto` | **특정 그룹의** 모집 공고를 새로 작성합니다. |
| 모집 공고 수정 | PATCH | /recruitments/{recruitmentId} | `UpdateRecruitmentRequest` | `RecruitmentDto` | 기존 모집 공고를 수정합니다. |
| 모집 공고 삭제 | DELETE | /recruitments/{recruitmentId} | | (204 No Content) | 모집 공고를 삭제합니다. |
| 모집 공고 수동 마감 | POST | /recruitments/{recruitmentId}/close | | (204 No Content) | 모집 공고를 수동으로 마감 처리합니다. |
| **내 그룹 공고 관리**| **GET** | **/groups/{groupId}/recruitments/mine** | | `List<MyRecruitmentDto>` | 특정 그룹 내에서 내가 관리할 수 있는 공고 목록을 조회합니다. |

### 8. DTO (Data Transfer Object) 정의
...
- **RecruitmentDetailDto** (상세용)
    ...
    - `endDate: DateTime`
    - `capacity: String`
- **MyRecruitmentDto** (관리용)
    - `recruitmentId: Long`
    - `title: String`
    - `status: String` (e.g., "Active", "Closed")
    - `startDate: DateTime`
    - `endDate: DateTime`
- **CreateRecruitmentRequest / UpdateRecruitmentRequest**
    - `title: String`
    - `content: String`
    - `tags: List<String>`
    - `startDate: DateTime`
