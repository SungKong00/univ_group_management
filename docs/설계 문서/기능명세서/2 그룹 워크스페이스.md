### **그룹/워크스페이스 기능 명세서 (v2.2 - API 응답 보강)**
...
| 워크스페이스 조회 | `GET` | `/api/groups/{groupId}/workspace` | 회원이 속한 그룹의 워크스페이스 정보를 조회. (응답에 `myMembership` 포함) |
...
- **WorkspaceDto**: `{ groupId, groupName, myRole, myMembership, notices, channels, members }`
