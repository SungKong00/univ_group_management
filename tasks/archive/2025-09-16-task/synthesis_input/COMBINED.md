...
- 프론트엔드:
  ...
  - 가입 신청 관리(목록/승인/반려) 최소 UI 제공
  - 멤버 관리(역할 변경/강제 탈퇴) 최소 UI 제공 + 역할 관리 폼(생성/삭제)
  - 그룹 삭제 UX: 바텀 시트 확인 + 빨간색 버튼(초기 단계의 단순 삭제 정책 안내)
  - 하위 그룹 생성 신청 폼 및 신청 관리(목록/승인/반려) 최소 UI 제공
  - 지도교수 관리(목록, 지정, 해제) 제공
  - 지도교수 관리 UI 개선(구현됨): 이름/이메일 검색 → 선택 지정 (간단 검색 API 연동)
  - 워크스페이스 1:1: 그룹 상세에서 워크스페이스를 전제로 채널/게시 UI 제공 (여러 채널 생성/관리)
...
### 미구현/개선 예정
- 하위 그룹 생성 신청 UI/관리 화면 (목록/승인/반려)
- 멤버 관리 UI (역할 변경, 강제 탈퇴) 및 해당 API 엔드포인트 정합성
- 지도교수 지정/해제 UI
- 워크스페이스/채널/게시글/댓글 백엔드 컨트롤러 구현 및 프론트 연동
- 하위 그룹 생성 신청 UI/관리(목록/승인/반려)
