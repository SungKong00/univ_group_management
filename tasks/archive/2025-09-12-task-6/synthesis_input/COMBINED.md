...
## 작업 목표
- [x] 목표 요약: 테스트 실패 문제 해결 (GroupServiceIntegrationTest.createGroup_Success 실패)
- [x] 성공 기준: 모든 테스트 통과

## 컨텍스트 요청 (태그, 파일, 영역)
- 태그: test, group, role, system
- 관련 소스/디렉토리: backend/src/test/kotlin, backend/src/main/kotlin
- 참고 문서: context/testing.md, context/group-management.md

## 개발 지시 (Claude Code용)
- 테스트 실패 원인 분석 후 수정
- 새로운 PROFESSOR 시스템 역할 추가로 인한 테스트 기댓값 업데이트 필요

## 작업 로그
- 2025-09-12 20:54 [Claude] 테스트 실행 및 실패 원인 파악 완료
  - GroupServiceIntegrationTest.createGroup_Success() 테스트 실패
  - 원인: 기본 시스템 역할 수 변경 (2개 → 3개)
  - 기존: OWNER, MEMBER (2개)
  - 현재: OWNER, PROFESSOR, MEMBER (3개)
- 2025-09-12 20:58 [Claude] 테스트 수정 진행 중
  - PROFESSOR 역할 추가를 반영하여 테스트 기댓값 수정
- 2025-09-12 21:00 [Claude] 테스트 수정 완료 및 전체 테스트 통과 확인
  - GroupServiceIntegrationTest.kt:91 라인 수정: hasSize(2) → hasSize(3)
  - 기댓값 업데이트: "OWNER", "MEMBER" → "OWNER", "PROFESSOR", "MEMBER"
  - 전체 45개 테스트 모두 통과 (100% 성공률)

## 변경 사항 요약
- 수정 파일:
  - backend/src/test/kotlin/org/castlekong/backend/service/GroupServiceIntegrationTest.kt (수정)
- 핵심 로직:
  - 기본 시스템 역할 개수 기댓값 2 → 3으로 변경
  - PROFESSOR 역할을 포함한 테스트 검증 로직 수정

## 컨텍스트 업데이트 요청
- context/testing.md에 PROFESSOR 시스템 역할 추가 관련 테스트 가이드 추가 요청
- metadata.json에 test 관련 태그 업데이트 요청
