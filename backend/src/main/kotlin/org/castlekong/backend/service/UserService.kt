...
class UserService(
    ...
    fun submitSignupProfile(userId: Long, req: SignupProfileRequest): User {
        ...
        val desiredRole = try { GlobalRole.valueOf(req.role) } catch (e: Exception) { GlobalRole.STUDENT }

        // 교수 지원은 PENDING 상태로 표시하고 실제 role은 STUDENT 유지
        val (finalRole, professorStatus) =
            if (desiredRole == GlobalRole.PROFESSOR) GlobalRole.STUDENT to ProfessorStatus.PENDING
            else desiredRole to null
    ...
    fun searchUsers(query: String, role: String?): List<User> {
        val roleEnum = try {
    ...
    fun getMyJoinRequests(userId: Long, status: String): List<GroupJoinRequestResponse> {
        val st = try { org.castlekong.backend.entity.GroupJoinRequestStatus.valueOf(status) } catch (e: Exception) { null }
        val requests = if (st != null) groupJoinRequestRepository.findByUserIdAndStatus(userId, st)
                       else groupJoinRequestRepository.findByUserIdAndStatus(userId, org.castlekong.backend.entity.GroupJoinRequestStatus.PENDING)
        return requests.map { r ->
            ...
            GroupJoinRequestResponse(
                ...
                reviewedBy = r.reviewedBy?.let { rb ->
    ...
    fun getMySubGroupRequests(userId: Long, status: String): List<SubGroupRequestResponse> {
        val st = try { org.castlekong.backend.entity.SubGroupRequestStatus.valueOf(status) } catch (e: Exception) { null }
        val requests = if (st != null) subGroupRequestRepository.findByRequesterIdAndStatus(userId, st)
                       else subGroupRequestRepository.findByRequesterIdAndStatus(userId, org.castlekong.backend.entity.SubGroupRequestStatus.PENDING)
        return requests.map { r ->
    ...
    @Transactional
    fun applyRole(userId: Long, role: String) {
        val user = userRepository.findById(userId)
            .orElseThrow { IllegalArgumentException("사용자를 찾을 수 없습니다: $userId") }
        val desiredRole = try { GlobalRole.valueOf(role) } catch (e: Exception) { GlobalRole.STUDENT }
        val updated = if (desiredRole == GlobalRole.PROFESSOR) {
