...
class ContentControllerTest {
    ...
    fun setUp() {
        ...
        // GroupInitializationRunner는 workspace를 생성하지 않으므로 수동 생성
        ...
        // GroupInitializationRunner가 생성한 기본 채널 조회 (workspace가 null인 채널)
        // TEXT 채널을 사용 (member도 POST_WRITE 권한 있음)
        val autoGeneratedChannels = channelRepository.findByGroup_Id(group.id)
        channel = autoGeneratedChannels.firstOrNull { ch -> ch.type == ChannelType.TEXT }
    ...
    private fun setupChannelPermissions(
        ...
        val ownerPermissions =
            setOf(
                ...
                org.castlekong.backend.entity.ChannelPermission.FILE_UPLOAD,
            )
        ...
        channelRoleBindingRepository.save(
            org.castlekong.backend.entity.ChannelRoleBinding.create(channel, ownerRole, ownerPermissions),
        )
        channelRoleBindingRepository.save(
            org.castlekong.backend.entity.ChannelRoleBinding.create(channel, memberRole, memberPermissions),
        )
    }
    ...
    inner class ChannelManagementTest {
        ...
        fun getChannelsByGroup_success() {
            ...
            mockMvc.perform(
                ...
                .andExpect(status().isOk)
                .andExpect(jsonPath("$.success").value(true))
                // 2개의 자동 생성 채널 (공지사항, 자유게시판) 확인
                .andExpect(jsonPath("$.data").isArray)
                .andExpect(jsonPath("$.data.length()").value(2))
                // 자유게시판 채널이 목록에 있는지 확인
                .andExpect(jsonPath("$.data[?(@.name == '자유게시판')].type").value("TEXT"))
        }
